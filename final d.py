...
sale_df["sale amount"] = pd.to_numeric(sale_df["sale amount"], errors="coerce")
rej_df["rej amt"] = pd.to_numeric(rej_df["rej amt"], errors="coerce")
...
fig_sale = go.Figure()
fig_sale.add_trace(go.Bar(x=sale_df["date"], y=sale_df["sale amount"], marker_color=bar_gradients, opacity=0.97))
fig_sale.update_layout(
    height=135, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
...
fig_rej.add_trace(go.Scatter(
    ...
    mode="lines+markers",
    marker=dict(size=9, color=BUTTERFLY_ORANGE, line=dict(width=1.5, color="#fff")),
...
# --- UI TEMPLATE (EXACT VS CODE GLASS STYLE) ---
