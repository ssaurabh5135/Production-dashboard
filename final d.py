...
# ------------------ COLORS (enhanced Neon Premium) ------------------
BUTTERFLY_ORANGE = "#ff9000"
BLUE = "#2488fa"
GREEN = "#00df6c"
ORANGE_GLOW = "#ffd278"
...
gauge = go.Figure(
    go.Indicator(
        ...
        gauge={
            ...
            "steps": [
                {"range": [0, 60], "color": "#d9fff0"},
                {"range": [60, 85], "color": "#73fabd"},
                {"range": [85, 100], "color": GREEN},
            ],
            "threshold": {
                "line": {"color": "#231", "width": 5},
                "value": achieved_pct_val,
            },
        },
    )
...
fig_sale.update_layout(
    ...
    yaxis=dict(showgrid=False, tickfont=dict(size=13, color="#fff"), automargin=True),
)
...
.container {{
    ...
    gap: 18px;
    row-gap: 30px;
    box-shadow: 0 4px 60px 0 rgba(90,110,140,0.14);
}}
.card {{
    background: linear-gradient(151deg,rgba(255,255,255,0.60) 12%,rgba(255,255,255,0.13) 83%);
    border-radius: var(--card-radius);
    border: 2.3px solid rgba(255,255,255,0.24);
    box-shadow: 0 10px 44px 0 rgba(80,100,150,0.18), 0 2.5px 8px rgba(0,0,0,0.13);
...
.value-green {{
    ...
    z-index: 2;
    text-shadow:
      0 2px 20px {ORANGE_GLOW}, /* Strong outer glow for all metrics */
      0 1px 14px rgba(255,255,255,0.28),
      0 0px 2px rgba(0,0,0,0.13),
      0 4px 30px rgba(0,0,0,0.17),
      0 0 6px #fff, /* fine crisp outer neon */
      0 0 1.5px #fff;
    transition: all 0.5s cubic-bezier(0.32, 0.72, 0.36, 0.96) !important;
    animation: value-pop 1.26s cubic-bezier(0.17, 0.85, 0.45, 1.04) both;
    filter: drop-shadow(0 3px 13px rgba(0,0,0,0.13));
    mix-blend-mode: lighten;
}}
.value-orange {{
    background-image: linear-gradient(97deg,#fff1c9 0%,#ffd77e 23%,#ffae32 62%,#ff9000 92%);
}}
.value-blue {{
    background-image: linear-gradient(92deg,#d9f8ff 6%,#b2ddff 32%,#2488fa 88%,#d6fbff 100%);
}}
.value-green {{
    background-image: linear-gradient(88deg,#afffee 14%,#71edb4 45%,#00df6c 100%);
    font-size: 62px !important;
}}
@keyframes value-pop {{
  0% {{ opacity:0; transform: translateY(18px) scale(0.98);}}
  68% {{ opacity:1; transform: translateY(-2px) scale(1.05);}}
...
.value-orange, .value-blue, .value-green {{
    ...
    animation:
      value-pop 1.26s cubic-bezier(0.14, 0.86, 0.29, 1.08) both,
