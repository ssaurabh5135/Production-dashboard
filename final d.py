...
# Styling for the date in last column with smaller font and blue color similar to sales trend graph axis
date_style = """
...
top_today_sale_fmt = format_inr(today_sale)
top_oee_fmt = f"{round(oee if pd.notna(oee) else 0, 1)}%"
left_rej_pct_fmt = f"{rej_pct: .1f}%"
bottom_rej_cum_fmt = format_inr(rej_cum)
top_date_fmt = latest[date_col].strftime("%d-%b-%Y")
...
<div class="container">
    ...
    <!-- Top left card: Today's Sale (replaces Date) -->
    <div class="card top-card">
      <canvas class="snow-bg" id="snowsale"></canvas>
      <div class="center-content">
        <div class="value-blue" id="salevalue">₹ {top_today_sale_fmt}</div>
    ...
    <div class="card top-card">
      ...
      <div class="center-content">
        <div class="value-orange" id="oeevalue">{top_oee_fmt}</div>
        <div class="title-black">OEE %</div>
      </div>
    </div>

    <!-- Top right card: Date with smaller font and blue color -->
    <div class="card top-card">
      <canvas class="snow-bg" id="snowdate"></canvas>
      <div class="center-content">
        <p style="{date_style}">{top_date_fmt}</p>
        <div class="title-black">Date</div>
      </div>
    </div>

    <!-- Middle left card: Rejection % -->
    <div class="card">
      <canvas class="snow-bg" id="snowrej"></canvas>
      <div class="center-content">
        <div class="value-orange" id="rejval">{left_rej_pct_fmt}</div>
        <div class="title-black">Rejection %</div>
      </div>
    </div>

    <!-- Middle middle card: Achieved % inside gauge speedometer -->
    <div class="card">
      <canvas class="snow-bg" id="snowspeed"></canvas>
      {gauge_html}
    </div>

    <!-- Middle right card: Rejection (Cumulative) -->
    <div class="card">
      <canvas class="snow-bg" id="snowrejcum"></canvas>
      <div class="center-content">
        <div class="value-orange" id="rejcum">{bottom_rej_cum_fmt}</div>
        <div class="title-black">Rejection (Cumulative)</div>
      </div>
    </div>

    <!-- Bottom left card: Sale Trend graph -->
    ...
    <!-- Bottom middle card: Rejection Amount -->
    <div class="card bottom-card">
    ...
    <!-- Bottom right card: Rejection Trend graph -->
    <div class="card bottom-card">
...
window.addEventListener("DOMContentLoaded",function() {{
    [
      ...
      "snowspeed","snowsalechart","snowrejchart","snowrejamt"
...
window.addEventListener("DOMContentLoaded", function() {{
    animateValue(document.getElementById('salevalue'), 0, parseInt("{top_today_sale_fmt.replace(',', '')}"), 1100, "", "₹ ");
    animateValue(document.getElementById('oeevalue'), 0, parseFloat("{top_oee_fmt.replace('%', '')}"), 1100, "%");
    animateValue(document.getElementById('rejval'), 0, parseFloat("{left_rej_pct_fmt.replace('%', '').strip()}"), 1100, "%");
    animateValue(document.getElementById('rejcum'), 0, parseInt("{bottom_rej_cum_fmt.replace(',', '')}"), 1100, "", "₹ ");
}});
</script>
</body>
</html>
