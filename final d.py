...
# ---------- Date selection BELOW cards ----------

# We will render the dashboard HTML first, then the calendar.
# But metrics must know the selected date, so we define it now.
min_d = df[date_col].min().date()
max_d = df[date_col].max().date()

# Use session_state so that when user picks a date below, the same
# date is available before HTML is rendered.
...
# ---------- HTML layout for cards (last card empty) ----------
...
# ---------- Calendar widget rendered BELOW the cards ----------
...
new_date = st.date_input(
    ...
    min_value=min_d,
    max_value=max_d,
    key="calendar_date_control",
)

# update session_state so next rerun uses this date
