...
if not all([date_col, today_col, oee_col, plan_col, rej_day_col, rej_pct_col, rej_cum_col, total_cum_col]):
    st.error("Required columns missing in Dashboard sheet.")
    st.stop()
...
# ---------- Month / TARGET_SALE table from A10:B... ----------

all_vals = dash_ws.get_values()
raw_target_rows = all_vals[9:] # from row 10 (index 9) down
...
def parse_month_name(mstr):
    if not isinstance(mstr, str):
        return None
    key = mstr.strip()[:3].lower()
    return month_map.get(key)

target_df["month_num"] = target_df["Month"].apply(parse_month_name)
target_df["TARGET_SALE"] = pd.to_numeric(
    target_df["TARGET_SALE"].astype(str).str.replace(",", ""),
    errors="coerce"
)
DEFAULT_TARGET_SALE = 19_92_00_000 # fallback if month not found

# ---------- Month list and selected month (no UI here) ----------
...
if "selected_ym" not in st.session_state or st.session_state["selected_ym"] not in available_periods:
    st.session_state["selected_ym"] = available_periods[-1]

selected_period = st.session_state["selected_ym"]

# ---------- Metrics for selected month + dynamic TARGET_SALE ----------
...
sel_month_num = selected_period.month
row_target = target_df[target_df["month_num"] == sel_month_num]
...
# ---------- SALE TREND GRAPH (safe for 1 day) ----------

sale_df_month["sale_lakh"] = sale_df_month["sale amount"] / 100000.0
n_bars = len(sale_df_month)
bar_gradients = ["rgb(34,139,230)"] if n_bars <= 1 else pc.n_colors(
    "rgb(34,139,230)", "rgb(79,223,253)", n_bars, colortype="rgb",
)
...
max_sale = sale_df_month["sale_lakh"].max()
fig_sale.update_layout(
    ...
    yaxis=dict(
        ...
        automargin=True,
        title="Lakh",
        range=[0, max(1, max_sale * 1.2)],
    ),
)

# ---------- REJECTION TREND GRAPH (safe for 1 day) ----------
...
max_rej = rej_df_month["rej_lakh"].max()
...
.container {{
    ...
    display: grid;
    grid-template-columns: 1fr 1fr 1fr; grid-template-rows: 130px 220px 140px 140px;
...
.value-green {{
    font-size: 42px !important; font-weight: 900;
    background: linear-gradient(180deg, var(--green1), var(--green2)); -webkit-background-clip: text; -webkit-text-fill-color: transparent;
}}
.title-black {{
    color: #f3f3f7 !important;
    font-size: 17px; font-weight: 800; margin-top: 6px; text-align: center;
}}
.chart-title-black {{
    position: absolute; top: 8px; left: 12px; color: #fff !important;
    font-size: 15px; font-weight: 700; z-index: 10;
}}
.chart-container {{
    width: 100%; height: 100%; display: block; padding:25px 5px 5px 5px; box-sizing: border-box;
}}
.snow-bg {{
    position: absolute; left: 0; top: 0; width: 100%; height: 100%; opacity: 0.5; pointer-events: none;
}}
.center-content {{
    display: flex; flex-direction: column; align-items: center; width: 100%; z-index: 5;
}}
.gauge-wrapper {{
...
# ---------- Month selector BELOW dashboard ONLY ----------

st.markdown("### Select month for dashboard")
new_period = st.selectbox(
    "Month",
    options=available_periods,
    index=available_periods.index(selected_period),
