...
body {{
    margin:0;padding:0;font-family:'Poppins',sans-serif;background:none !important;color:#091128;
}}
.container {{
    box-sizing: border-box;width: 100vw;height: 100vh;padding: 5vw;
    display: grid;grid-template-columns: 1fr 1fr 1fr;grid-template-rows: 150px 170px 170px;
    gap: 18px 18px;row-gap: 30px;max-width: 1700px;max-height: 900px;margin: auto;
}}
.card {{
    background: linear-gradient(184deg,rgba(255,255,255,0.13) 12%,rgba(255,255,255,0.04) 83%);
    border-radius: 16px;box-shadow: 0 6px 18px rgba(4,8,15,0.13);border: 1px solid rgba(255,255,255,0.08);
    backdrop-filter: blur(6px) saturate(120%);-webkit-backdrop-filter: blur(6px);
...
.value-orange, .value-blue {{
    font-size:54px!important; font-family:'Poppins','Segoe UI',Arial,sans-serif; font-weight:900!important;
...
.value-orange {{
    ...
    -webkit-text-stroke:1.2px #b96000; filter:drop-shadow(0 4px 18px #fc7d1b);
    border-radius:10px; animation:popval 1.18s cubic-bezier(0.14,0.86,0.29,1.08) both, shimmer 3.2s linear infinite;
    background-size:200% 100%; text-align:center;
}}
.value-blue {{
    ...
    -webkit-text-stroke:1.2px #1661a2; filter:drop-shadow(0 4px 18px #228be6);
    border-radius:10px; animation:popval 1.1s cubic-bezier(0.14,0.86,0.29,1.08) both, shimmer 3.2s linear infinite;
    background-size:200% 100%; text-align:center;
}}
.value-green {{
    ...
    color:transparent!important; text-shadow:0 3px 8px #fffbe8,0 5px 16px #00df6c,0 10px 30px #aef9e2;
    -webkit-text-stroke:1.2px #1a8d56; filter:drop-shadow(0 4px 16px #00df6c);
    border-radius:10px; animation:popval 1.1s cubic-bezier(.14,.86,.29,1.08) both, shimmer 3.4s linear infinite;
...
.chart-container {{
    width:100%; height:110px; overflow:hidden; box-sizing:border-box; margin:0; padding:0; display:block;
}}
.center-content {{
...
<div class="container">
    ...
    <!-- Top row -->
    <div class="card"><canvas class="snow-bg" id="snowsale"></canvas>
      <div class="center-content"><div class="value-blue" id="salevalue">₹ {top_today_sale}</div>
        <div class="title-black">Today's Sale</div></div>
    </div>
    <div class="card"><canvas class="snow-bg" id="snowrejamt"></canvas>
      <div class="center-content"><div class="value-orange" id="rejamt">{format_inr(rej_day)}</div>
        <div class="title-black">Rejection Amount</div></div>
    </div>
    <div class="card"><canvas class="snow-bg" id="snowdate"></canvas>
      <div class="center-content"><div class="value-orange" id="datevalue">{top_date}</div>
        <div class="title-black">Date</div></div>
    </div>

    <!-- Middle row -->
    <div class="card"><canvas class="snow-bg" id="snowspeed"></canvas>
      {gauge_html}
    </div>
    <div class="card"><canvas class="snow-bg" id="snowrej"></canvas>
      <div class="center-content"><div class="value-orange" id="rejval">{left_rej_pct}</div>
        <div class="title-black">Rejection %</div></div>
    </div>
    <div class="card"><canvas class="snow-bg" id="snowcopq"></canvas>
      <div class="center-content"><div class="value-blue" id="copqval">COPQ Updating...</div>
        <div class="title-black"></div></div>
    </div>

    <!-- Bottom row -->
    <div class="card bottom-card"><canvas class="snow-bg" id="snowsalechart"></canvas>
      <div class="chart-title-black">Sale Trend</div>
      <div id="sale_chart_container" class="chart-container">{sale_html}</div>
    </div>
    <div class="card bottom-card"><canvas class="snow-bg" id="snowrejchart"></canvas>
      <div class="chart-title-black">Rejection Trend</div>
      <div id="rej_chart_container" class="chart-container">{rej_html}</div>
    </div>
    <div class="card bottom-card"><canvas class="snow-bg" id="snowrejcum"></canvas>
...
window.addEventListener("DOMContentLoaded",function() {{
    [
      "snowsale","snowrejamt","snowdate",
      "snowspeed","snowrej","snowcopq",
      "snowsalechart","snowrejchart","snowrejcum"
