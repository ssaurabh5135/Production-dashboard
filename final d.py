...
def load_monthly_targets(sheet):
    ...
    month_to_target = {}
    for row in vals[1:]: # skip header row A10 and B10
        if len(row) >= 2:
            month = row[0].strip()
            target_val = pd.to_numeric(row[1].replace(",", ""), errors='coerce')
            month_to_target[month] = target_val if target_val is not None else 0
...
# Load monthly target sales from Dashboard sheet
month_targets = load_monthly_targets(dash_ws)

# ---------- Sales Report Backup Data ----------
...
selected_month = st.selectbox("Select Month", options=["All"] + month_options)
...
if selected_month == "All":
    ...
    total_cum = filtered_sale_df["sale amount"].sum()
    latest_mask = (df[date_col].dt.month == month_num)
    if latest_mask.any():
        latest = df[latest_mask].iloc[-1]
    else:
        latest = df.iloc[-1]
...
selected_target_sale = TARGET_SALE if selected_month == "All" else month_targets.get(selected_month, 0)
if not selected_target_sale:
    selected_target_sale = 1 # prevent zero division

achieved_pct_val = round(total_cum / selected_target_sale * 100, 2) if selected_target_sale else 0
...
sale_df_plot = filtered_sale_df.copy()
sale_df_plot["sale_lakh"] = sale_df_plot["sale amount"] / 100000.0
bar_gradients = pc.n_colors("rgb(34,139,230)", "rgb(79,223,253)", len(sale_df_plot), colortype="rgb")
fig_sale = go.Figure()
fig_sale.add_trace(
    go.Bar(
        x=sale_df_plot["date"],
        y=sale_df_plot["sale_lakh"],
...
rej_df_plot = filtered_rej_df.copy()
rej_df_plot["rej_lakh"] = rej_df_plot["rej amt"] / 1000.0
fig_rej = go.Figure()
fig_rej.add_trace(
    go.Scatter(
        x=rej_df_plot["date"],
        y=rej_df_plot["rej_lakh"],
        mode="lines+markers",
...
# ---------- Dashboard UI HTML and CSS (Unchanged except month selector added below) ----------
...
# The month selector is already rendered above by st.selectbox 
