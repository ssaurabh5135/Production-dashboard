...
# -------------------- Determine Current Month for Default --------------------
# Use latest date from dashboard as default
if not df.empty:
    latest_date = df[date_col].max()
    default_month_name = latest_date.strftime("%b")
    if default_month_name not in month_options:
        default_month_name = month_options[-1]  # fallback
else:
    default_month_name = month_options[-1]
# -------------------- Dashboard Rendering --------------------
def render_dashboard(selected_month):
    # Map month name to month number
    selected_month_num = pd.to_datetime(selected_month, format='%b').month

    # Filter dashboard and sales/rejection data for the selected month
    ...
    html_template = f"""
    <style>
    body, .stApp {{ background: none !important; margin:0; padding:0; overflow:hidden; }}
    .container {{ display:grid; grid-template-columns:1fr 1fr 1fr; grid-template-rows:130px 130px 140px 140px; gap:24px; max-width:1700px; max-height:900px; margin:auto; padding:60px 60px 0 60px !important; }}
...
selected_month = st.selectbox("Select Month to View Data for", month_options, index=month_options.index(default_month_name))
