...
<div class="container">
    ...
    <!-- COL 1: Today's Sale (top), Gauge (middle), Sale Trend (bottom) -->
    ...
    <!-- COL 2: Rejection Amount (top), Rejection % (middle), Rejection Trend (bottom) -->
    <div class="card top-card">
      <canvas class="snow-bg" id="snowrejamt"></canvas>
      <div class="center-content">
        <div class="value-orange" id="rejamtval">₹ {format_inr(rej_day)}</div>
    ...
    <!-- COL 3: Date (top), OEE % (middle), Rejection Cumulative (bottom) -->
    <div class="card top-card">
...
window.addEventListener("DOMContentLoaded", function() {{
    ...
    animateValue(document.getElementById('rejval'), 0, parseFloat("{left_rej_pct.replace('%', '').trim()}"), 1100, "%");
    ...
    animateValue(document.getElementById('rejamtval'), 0, parseInt("{format_inr(rej_day).replace(',', '')}"), 1100, "", "₹ ");
}});
</script>
</body>
</html>
