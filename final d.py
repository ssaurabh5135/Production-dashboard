...
fig_sale.update_layout(
    ...
    plot_bgcolor="rgba(0,0,0,0)",
    height=135,
    xaxis=dict(showgrid=False, tickfont=dict(size=12), tickangle=-45, automargin=True, title=""),
...
function makeSnow(canvas) {{
    ...
    var sn = [];
    for(var i=0;i<40;i++) sn.push({{x:Math.random()*w,y:Math.random()*h,r:1.1+Math.random()*2,dx:0,dy:1+Math.random()*1.65}});
    function loop(){{
