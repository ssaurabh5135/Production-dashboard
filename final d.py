...
TARGET_SALE = 16_68_00_000 # default target in ₹ if month not found
...
# ---------- Find required columns in Dashboard ----------
...
# ---------- Convert types & add month labels ----------
...
if df.empty:
    st.error("No valid dates in Dashboard data.")
    st.stop()

df["month_label"] = df[date_col].dt.strftime("%b")

# ---------- Read Month-wise TARGET_SALE from Dashboard A10:B... ----------

month_target_map = {}
month_header_row = None

for i, r in enumerate(rows):
    if len(r) >= 2 and str(r[0]).strip().lower() == "month":
        # Optionally check "TARGET_SALE" text in r[1], but not strictly required
        month_header_row = i
        break

if month_header_row is not None:
    for r in rows[month_header_row + 1:]:
        if not r or len(r) < 2:
            continue
        month_name = str(r[0]).strip()
        if not month_name:
            continue
        val_str = str(r[1]).replace(",", "").strip()
        try:
            month_target_map[month_name] = float(val_str)
        except Exception:
            continue

# ---------- Month order & selected month (from session_state) ----------

month_order = (
    df.groupby("month_label")[date_col]
    .min()
    .sort_values()
    .index
    .tolist()
...
latest_month = df.loc[df[date_col].idxmax(), "month_label"]

selected_month = st.session_state.get("selected_month", latest_month)
if selected_month not in month_order:
    selected_month = latest_month

# ---------- Filter Dashboard df by selected month ----------

df_month = df[df["month_label"] == selected_month].copy()
if df_month.empty:
    # fallback to full data if no rows for that month (to avoid crash)
    df_month = df.copy()

df_month = df_month.sort_values(date_col)
latest = df_month.iloc[-1]

# Target sale for selected month
target_sale_for_month = month_target_map.get(selected_month, TARGET_SALE)

# ---------- Metrics from Dashboard (for selected month) ----------
...
cum_series = df_month[total_cum_col].dropna()
total_cum = cum_series.iloc[-1] if not cum_series.empty else 0
achieved_pct_val = round(total_cum / target_sale_for_month * 100, 2) if target_sale_for_month else 0

if copq_col and pd.notna(latest.get(copq_col, None)):
    copq_display = format_inr(latest[copq_col])
else:
    copq_display = "..."

if copq_cum_col and pd.notna(latest.get(copq_cum_col, None)):
...
# ---------- Gauge (Achieved % for selected month) ----------
...
if sr_rows and len(sr_rows) > 1:
    ...
    rej_records = []
    for r in sr_rows[1:]:
        # Sale_summery table (example assumption)
        if len(r) >= 3:
        ...
        # Rejection_summery table (example assumption)
        if len(r) >= 12:
...
# Fallback to Dashboard data if Sales Report is empty
...
# Filter trends by selected month (with fallback)

sale_df["month_label"] = sale_df["date"].dt.strftime("%b")
rej_df["month_label"] = rej_df["date"].dt.strftime("%b")

sale_df_month = sale_df[sale_df["month_label"] == selected_month].copy()
if sale_df_month.empty:
    sale_df_month = sale_df.copy()

rej_df_month = rej_df[rej_df["month_label"] == selected_month].copy()
if rej_df_month.empty:
    rej_df_month = rej_df.copy()

# ---------- SALE TREND GRAPH (Lakhs, with safe gradient) ----------

sale_df_month["sale_lakh"] = sale_df_month["sale amount"] / 100000.0

if len(sale_df_month) <= 1:
    bar_gradients = ["rgb(34,139,230)"] * max(len(sale_df_month), 1)
else:
...
# ---------- REJECTION TREND GRAPH (K) ----------

rej_df_month["rej_lakh"] = rej_df_month["rej amt"] / 1000.0
...
# ---------- Streamlit + HTML (layout same as your code) ----------
...
# ---------- Month selector at bottom (controls next rerun) ----------

idx = month_order.index(selected_month) if selected_month in month_order else 0
st.selectbox(
    "Select Month",
    month_order,
    index=idx,
...
# DASHBOARD_SHEET = "Dashboard"
# SALES_REPORT_SHEET = "Sales Report"
# TARGET_SALE = 16_68_00_000 # default target in ₹ (used only if month-wise not found)
...
# for i, r in enumerate(rows):
#     if len(r) >= 2 and str(r[0]).strip().lower() == "month" and str(r[1]).strip().replace(" ", "").lower().startswith("target"):
#         month_header_row = i
#         break
...
# # ---------- Find required columns in Dashboard df ----------
...
# # ---------- Prepare Dashboard df with dates & months ----------
...
# # Month label like "Nov", "Dec"
# df["month_label"] = df[date_col].dt.strftime("%b")

# # Order months by first date occurrence
# month_order = (
#     df.groupby("month_label")[date_col].min().sort_values().index.tolist()
# )

# # Default month = latest date's month
# latest_month_default = df.loc[df[date_col].idxmax(), "month_label"]

# if "selected_month" not in st.session_state:
#     st.session_state["selected_month"] = latest_month_default

# selected_month = st.session_state["selected_month"]
# if selected_month not in month_order:
#     selected_month = latest_month_default
#     st.session_state["selected_month"] = selected_month

# # Filter dashboard df by selected month
# df_month = df[df["month_label"] == selected_month].copy()
# if df_month.empty:
#     # fallback: use all data if no data for selected month (should not normally happen)
#     df_month = df.copy()
...
# # ---------- Sales Report for Trends (full data) ----------
...
# sale_df_all = sale_df.copy()
# rej_df_all = rej_df.copy()

# sale_df = sale_df_all[sale_df_all["date"].dt.strftime("%b") == selected_month].copy()
# if sale_df.empty:
#     sale_df = sale_df_all.copy()

# rej_df = rej_df_all[rej_df_all["date"].dt.strftime("%b") == selected_month].copy()
# if rej_df.empty:
#     rej_df = rej_df_all.copy()

# # ---------- SALE TREND GRAPH (Lakhs, all days of selected month) ----------
...
# # ---------- REJECTION TREND GRAPH (K, all days of selected month) ----------
...
# bottom_rej_cum = format_inr(rej_cum)
# total_cum_disp = format_inr(total_cum)
# # ---------- Streamlit + HTML (layout unchanged) ----------
...
# # ---------- Month selection at bottom (controls next rerun) ----------

# st.selectbox(
#     "Select Month",
#     month_order,
#     index=month_order.index(st.session_state["selected_month"]),
#     key="selected_month",
# )
