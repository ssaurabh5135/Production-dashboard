...
# --- UPDATE: last 3 months only ---
all_months = sorted(month_targets.keys(), key=lambda m: pd.to_datetime(m, format='%b').month)
month_options = all_months[-3:]  # last 3 months
...
def render_dashboard(selected_month):
    ...
    if target_sale <= 0:
        target_sale = 1
    achieved_pct_val = min(round(total_sales_filtered / target_sale * 100, 2), 100)  # --- CAP 100% ---
    ...
    .container {{ display:grid; grid-template-columns:1fr 1fr 1fr; grid-template-rows:130px 130px 140px 140px; gap:24px; max-width:1700px; max-height:900px; margin:auto; padding:60px; }}
    .card {{ position:relative; border-radius:20px; display:flex; flex-direction:column; justify-content:center; align-items:center; backdrop-filter:blur(12px) saturate(180%); background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15); box-shadow:0 0 15px rgba(255,255,255,0.28),0 10px 30px rgba(0,0,0,0.5), inset 0 0 20px rgba(255,255,255,0.12); overflow:hidden; }}
    .value-blue {{ font-size:42px;font-weight:900; background:linear-gradient(180deg,#8ad1ff,#4ca0ff,#0d6efd); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }}
    .value-orange {{ font-size:42px;font-weight:900; background:linear-gradient(180deg,#ffd699,#ff9334,#ff6a00); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }}
