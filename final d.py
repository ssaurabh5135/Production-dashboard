...
gauge = go.Figure(
    go.Indicator(
        ...
        number={"suffix": "%", "font": {"size": 36, "color": "#009e4f", "family": "Poppins", "weight": "bold"}},
        domain={"x": [0, 1], "y": [0, 1]},
        gauge={
            "shape": "angular",
...
left_rej_amt = format_inr(latest[rej_day_col])
left_rej_pct = f"{ensure_pct(latest[rej_pct_col]):.1f}%"
bottom_rej_cum = format_inr(latest[rej_cum_col])
total_cum_disp = format_inr(latest[total_cum_col])
