...
# --- FIXED MONTH TARGETS ---
month_targets_vals = dash_ws.get_values('A10:B21')  # A10:B21 to cover all months
month_targets = {}
for row in month_targets_vals[1:]:
    if len(row) >= 2:
        month_name = row[0].strip()
        try:
            target_val = float(str(row[1]).replace(",", "").strip())
            if target_val <= 0:
                target_val = 1
        except Exception:
            target_val = 1
        month_targets[month_name] = target_val
...
.container {{
    box-sizing: border-box; width:100%; height:100vh; padding:60px 60px 0 60px !important;
    display:grid; grid-template-columns:1fr 1fr 1fr; grid-template-rows:130px 130px 140px 140px;
    gap:24px; max-width:1700px; max-height:900px; margin:auto;
}}
.card {{
    position:relative; border-radius:20px; padding:0; display:flex; flex-direction:column; justify-content:center;
    align-items:center; backdrop-filter: blur(12px) saturate(180%); background: rgba(255,255,255,0.08);
    border:1px solid rgba(255,255,255,0.15); box-shadow: 0 0 15px rgba(255,255,255,0.28), 0 10px 30px rgba(0,0,0,0.5), inset 0 0 20px rgba(255,255,255,0.12);
    overflow:hidden;
}}
...
<div class="card"><div class="value-green">{top_oee}</div><div class="title-black">OEE</div></div>
<div class="card"><div class="value-orange">{left_rej_amt} / {left_rej_pct}</div><div class="title-black">Rejection (Day)</div></div>
<div class="card chart-container">{sale_html}</div>
<div class="card chart-container">{rej_html}</div>
<div class="card chart-container">{gauge_html}</div>
<div class="card"><div class="value-blue">{bottom_rej_cum}</div><div class="title-black">Rejection (Cum)</div></div>
<div class="card"><div class="value-orange">{total_cum_disp}</div><div class="title-black">Total Sale (Cum)</div></div>
<div class="card"><div class="value-green">{inventory_disp}</div><div class="title-black">Inventory Value</div></div>
</div></body></html>"""
