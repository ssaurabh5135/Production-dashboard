...
if not all([date_col, today_col, oee_col, plan_col, rej_day_col, rej_pct_col, rej_cum_col, total_cum_col]):
    st.error("One or more required dashboard columns are missing in Dashboard sheet.")
    st.stop()
...
# ---------- Build available months and set selected month (selector renders at bottom only)

df["year_month"] = df[date_col].dt.to_period("M")
all_periods = sorted(df["year_month"].unique().tolist())
def ym_label(p):
    return p.strftime("%b %Y")

if "selected_ym" not in st.session_state or st.session_state["selected_ym"] not in all_periods:
    st.session_state["selected_ym"] = all_periods[-1]
selected_period = st.session_state["selected_ym"]

# ---------- Metrics and month target ----------
...
# ---------- SALE TREND GRAPH (safe for any points) ----------
...
# ---------- REJECTION TREND GRAPH (safe for any points) ----------
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="center-content">
            <div class="value-blue">₹ {top_today_sale}</
...
# Render the dashboard (cards). Month selector is NOT here, so cards don't move.
...
new_period = st.selectbox(
    "Month",
    options=all_periods,
    index=all_periods.index(selected_period),
