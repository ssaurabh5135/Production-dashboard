...
# from datetime import datetimest.set_page_config(page_title="Factory Dashboard (Exact Layout)", layout="wide")IMAGE_PATH = "black.jpg"
# SPREADSHEET_ID = "168UoOWdTfOBxBvy_4QGymfiIRimSO2OoJdnzBDRPLvk"
# DASHBOARD_SHEET = "Dashboard"
# SALES_REPORT_SHEET = "Sales Report"
# TARGET_SALE = 19_92_00_000 # yearly target in ₹---------- Helpers ----------def load_image_base64(path: str) -> str:
# try:
# data = Path(path).read_bytes()
# return base64.b64encode(data).decode()
# except Exception:
# return ""def format_inr(n):
# try:
...
# )
# return rest + last3def ensure_pct(x):
# try:
# v = float(str(x).replace("%", "").replace(",", ""))
# except Exception:
# return 0.0
# return v * 100 if v <= 5 else vdef find_col(df, target):
# norm_target = (
# target.lower()
# .replace(" ", "")
# .replace("%", "")
...
# return c
# return None---------- Auth & Dashboard ----------try:
...
# st.stop()if not rows or len(rows) < 2:
...
# df.columns = df.columns.astype(str)date_col = find_col(df, "date")
...
# total_cum_col = find_col(df, "total sales (cumulative)") or find_col(df, "total sales cumulative")if not all([date_col, today_col, oee_col, plan_col, rej_day_col, rej_pct_col, rej_cum_col, total_cum_col]):
# st.error("One or more required dashboard columns are missing in Google Sheet.")
# st.stop()copq_col = find_col(df, "copq")
# copq_cum_col = find_col(df, "copq cumulative") or find_col(df, "copqcumulative")df[date_col] = pd.to_datetime(df[date_col], errors="coerce")
# for c in df.columns:
# if c != date_col:
# df[c] = pd.to_numeric(df[c].astype(str).str.replace(",", ""), errors="coerce")df = df.dropna(subset=[date_col]).sort_values(date_col)---------- Month selector (small button) ----------Create list of unique year-months from data, latest firstdf["year_month"] = df[date_col].dt.to_period("M")
# unique_periods = sorted(df["year_month"].unique().tolist())
# if unique_periods:
# default_period = unique_periods[-1]
# else:
# default_period = datetime.today().strftime("%Y-%m")Show as small selectbox in sidebar-like top barmonth_label_map = {p: p.strftime("%b %Y") for p in unique_periods}
# selected_period = st.selectbox(
# "Select month",
# options=unique_periods,
# index=unique_periods.index(default_period),
# format_func=lambda p: month_label_map.get(p, str(p)),
# key="month_selector",
# )Filter df to selected month (start from day 1)df_month = df[df["year_month"] == selected_period].copy()
# if df_month.empty:
# st.error("No data for selected month.")
# st.stop()---------- Recalculate metrics for selected month ----------latest = df_month.iloc[-1]today_sale = latest[today_col]
# raw_oee = latest[oee_col]
# oee = ensure_pct(raw_oee)
# raw_plan = latest[plan_col]
# plan_vs_actual = ensure_pct(raw_plan)rej_day_amount = latest[rej_day_col]
# raw_rej_pct = latest[rej_pct_col]
# rej_pct = ensure_pct(raw_rej_pct)
# rej_cum = latest[rej_cum_col]cum_series = df_month[total_cum_col].dropna()
# total_cum = cum_series.iloc[-1] if not cum_series.empty else 0
# achieved_pct_val = round(total_cum / TARGET_SALE * 100, 2) if TARGET_SALE else 0if copq_col and pd.notna(latest[copq_col]):
# copq_display = format_inr(latest[copq_col])
# else:
# copq_display = "..."if copq_cum_col and pd.notna(latest[copq_cum_col]):
# copq_cum_display = format_inr(latest[copq_cum_col])
# else:
# copq_cum_display = "..."BUTTERFLY_ORANGE = "#fc7d1b"
# BLUE = "#228be6"
# GREEN = "#009e4f"---------- Gauge ----------gauge = go.Figure(
# go.Indicator(
# mode="gauge+number",
...
# height=130,
# )gauge_html = gauge.to_html(include_plotlyjs="cdn", full_html=False)---------- Sales Report for Trends ----------try:
# sr_ws = sh.worksheet(SALES_REPORT_SHEET)
# sr_rows = sr_ws.get_values()
# except Exception:
# sr_rows = []sale_df = None
# rej_df = Noneif sr_rows and len(sr_rows) > 1:
# sale_records = []
# rej_records = []
# for r in sr_rows[1:]:
# if len(r) >= 3:
# date_str = (r or "").strip()
# sales_type = (r� or "").strip().upper()
# sale_amt = r
# if date_str and sales_type == "OEE":
# sale_records.append({"date": date_str, "sale amount": sale_amt})
# if len(r) >= 12:
# rej_date_str = (r or "").strip()
# rej_amt = r
...
# if rej_records:
# rej_df = pd.DataFrame(rej_records)if sale_df is None or sale_df.empty:
# sale_df = pd.DataFrame({"date": df[date_col], "sale amount": df[today_col]})if rej_df is None or rej_df.empty:
# rej_df = pd.DataFrame({"date": df[date_col], "rej amt": df[rej_day_col]})sale_df["date"] = pd.to_datetime(sale_df["date"], errors="coerce")
# sale_df["sale amount"] = pd.to_numeric(
# sale_df["sale amount"].astype(str).str.replace(",", ""), errors="coerce"
# ).fillna(0)
# sale_df = sale_df.dropna(subset=["date"]).sort_values("date")rej_df["date"] = pd.to_datetime(rej_df["date"], errors="coerce")
# rej_df["rej amt"] = pd.to_numeric(
# rej_df["rej amt"].astype(str).str.replace(",", ""), errors="coerce"
# ).fillna(0)
# rej_df = rej_df.dropna(subset=["date"]).sort_values("date")Filter trend data to selected month as wellsale_df["year_month"] = sale_df["date"].dt.to_period("M")
# rej_df["year_month"] = rej_df["date"].dt.to_period("M")sale_df_month = sale_df[sale_df["year_month"] == selected_period].copy()
# rej_df_month = rej_df[rej_df["year_month"] == selected_period].copy()if sale_df_month.empty:
# sale_df_month = sale_df.copy()
# if rej_df_month.empty:
# rej_df_month = rej_df.copy()
# ---------- SALE TREND GRAPH (Lakhs, selected month) ----------

# sale_df_month["sale_lakh"] = sale_df_month["sale amount"] / 100000.0bar_gradients = pc.n_colors(
# "rgb(34,139,230)",
# "rgb(79,223,253)",
# len(sale_df_month),
# colortype="rgb",
...
# fig_sale.add_trace(
# go.Bar(
# x=sale_df_month["date"],
# y=sale_df_month["sale_lakh"],
# marker_color=bar_gradients,
# marker_line_width=0,
# opacity=0.97,
# hovertemplate="Date: %{x|%d-%b}Sale: %{y:.2f} Lakh",
# )
...
# title="Lakh",
# ),
# )---------- REJECTION TREND GRAPH (K, selected month) ----------rej_df_month["rej_lakh"] = rej_df_month["rej amt"] / 1000.0fig_rej = go.Figure()fig_rej.add_trace(
# go.Scatter(
# x=rej_df_month["date"],
# y=rej_df_month["rej_lakh"],
# mode="lines+markers",
# marker=dict(
# size=8,
...
# hoverinfo="x+y",
# opacity=1,
# hovertemplate="Date: %{x|%d-%b}Rejection: %{y:.2f} K",
# )
# )fig_rej.add_trace(
# go.Scatter(
# x=rej_df_month["date"],
...
# rej_html = fig_rej.to_html(include_plotlyjs="cdn", full_html=False)bg_b64 = load_image_base64(IMAGE_PATH)top_today_sale = format_inr(today_sale)
...
# total_cum_disp = format_inr(total_cum)---------- Streamlit + HTML ----------st.markdown(
# f"""
