...
required_cols = [date_col, today_col, oee_col, plan_col, rej_day_col,
                 rej_pct_col, rej_cum_col, total_cum_col]
...
if sr_rows and len(sr_rows) > 1:
    for r in sr_rows[1:]:
        # OEE Sales
        if len(r) >= 3:
            date_str, sales_type, sale_amt = (r[0] or "").strip(), (r[1] or "").strip().upper(), r[2]
            if date_str and sales_type == "OEE":
                sale_records.append({"date": date_str, "sale amount": sale_amt})
        # Rejection Data
        if len(r) >= 12:
            rej_date_str, rej_amt = (r[10] or "").strip(), r[11]
...
month_targets = {row[0].strip(): float(row[1].replace(",", "")) if len(row) > 1 and row[1].strip() else 1
...
def render_dashboard(selected_month):
    ...
    bar_gradients = pc.n_colors(
        "rgb(34,139,230)", "rgb(79,223,253)", num_colors, colortype="rgb"
    )
    ...
    fig_sale.add_trace(
        go.Bar(
            ...
            y=sale_filtered["sale amount"] / 100000.0,  # Lakh
    ...
    fig_sale.update_layout(
        ...
        xaxis=dict(showgrid=False, tickfont=dict(size=10), tickangle=-45,
                   automargin=True, tickformat="%d", dtick="D1"),
    ...
    .container {{
        width:100%; height:100vh; padding:60px 60px 0 60px; display:grid;
    ...
    .value-blue {{ font-size:42px;font-weight:900;background:linear-gradient(180deg,#8ad1ff,#4ca0ff,#0d6efd);
        -webkit-background-clip:text; -webkit-text-fill-color:transparent; }}
    .value-orange {{ font-size:42px;font-weight:900;background:linear-gradient(180deg,#ffd699,#ff9334,#ff6a00);
        -webkit-background-clip:text; -webkit-text-fill-color:transparent; }}
    .value-green {{ font-size:42px;font-weight:900;background:linear-gradient(180deg,#a6ffd9,#00d97e);
        -webkit-background-clip:text; -webkit-text-fill-color:transparent; }}
    ...
    <div class="container">
        <!-- Row 1 -->
        <div class="card"><div class="center-content">
        ...
        <!-- Row 3 -->
        <div class="card"><div class="chart-title-black">Sale Trend</div>
            <div class="chart-container">{sale_html}</div>
        </div>
        <div class="card"><div class="chart-title-black">Rejection Trend</div>
